...
ump_secure_id ump_arch_dmabuf_import(int fd)
{
	_ump_uk_dmabuf_s dd_dmabuf_arg;

	dd_dmabuf_arg.ctx = ump_uk_ctx;
	dd_dmabuf_arg.fd = fd;

	UMP_DEBUG_PRINT(4, ("ump_dmabuf_import: 0x%x\n", fd));
	_ump_uku_dmabuf_import(&dd_dmabuf_arg);
	return (ump_secure_id)dd_dmabuf_arg.secure_id;
}
#endif
